...
void main(int argc,char** argv)
{
	...
	do
	{
		...
		if(0==strcmp(buf,"help"))
		{
			...
			cout << "n[algeb]=[annotation] : annotate move ( example 'ng1f3=!' )" << endl;
			cout << "d : use default annotations" << endl;
			#ifdef ALLOW_SAVE
		...
		if(buf[0]=='n')
		{
			...
			if((buf[5]=='=')||(buf[6]=='='))
			{
				...
				{
					strcpy_s(startup_message,"move annotated ok");
				}
				else
				{
					strcpy_s(startup_message,"error annotating move ( possible causes: move invalid or out of memory )");
				}
			}
			else
			{
				strcpy_s(startup_message,"wrong annotation syntax, use n[algeb]=[annotation]");
			}
